...
from utils.utils import compute_prob_X

def log_likelihood_mu(mu, e, e_blocks, blocks, x):
    p_x_1_mu = compute_prob_X(mu, e, e_blocks, blocks, list(x.shape))
    p_x_0_mu = 1-p_x_1_mu
...
def mh_accept_mu(mu, mu_new, e, e_blocks, blocks, x):
...
def gibbs_sample_x(lotus_n_papers, mu, e, e_blocks, gamma, delta, blocks):
    p_x_1 = compute_prob_X(mu, e, e_blocks, blocks, list(lotus_n_papers.shape))
    #calculate prior
    p_x_0 = 1 - p_x_1
    
    #calculate likelihood of data given x
    likelihood_x_0 = likelihood(lotus_n_papers, np.zeros_like(p_x_1), gamma, delta)
    likelihood_x_1 = likelihood(lotus_n_papers, np.ones_like(p_x_1), gamma, delta)
    ...
    for i in range(n_iter):
        ...
        for m in range(len(mu_arrays)):
            for j in range(len(mu_arrays[m])):
                ...
                if mh_accept_mu(mu.flatten(), mu_new.flatten(), e, e_blocks, blocks, x):
                    mu = mu_new
